...
public class GroupService {
    ...
    public List<GroupJoinRequest> getGroupJoinRequests(Long groupNumber) throws GroupNotJoinException {
        Groups findGroups = groupJpaRepository.findById(groupNumber)
                .orElseThrow((GroupNotJoinException::new));

        List<GroupJoinRequest> findRequests = groupJoinRequestJpaRepository.findByGroups(findGroups);
     ...
     * 그룹 생성자인지 여부 확인
     */
    public boolean isGroupCreator(Long groupNumber, Long userNumber) throws GroupNotFoundException {
        Groups group = groupJpaRepository.findById(groupNumber)
                .orElseThrow(GroupNotFoundException::new);

        return group.getCreator().getUserNumber().equals(userNumber);
    }
